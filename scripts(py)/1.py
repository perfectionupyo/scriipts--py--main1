...
class BigT:
    def __init__(self):
        self.state = None
        self.state_back = {0:0, 2:0, 3:1, 4:1, 5:1, 6:4, 7:5, 8:6, 9:7}
        self.user_data = {'user_id':None, 'age':None, 'bad':None, 'minus':None, 'gender':None, 'weight':0, 'height':0, 'BMI':0, 'goal':None, 'new':None}
    ...
    def start_command(self, update, context: CallbackContext):
        ...
        context.bot.send_message(chat_id=update.effective_chat.id, text=self.text.greeting,
            reply_markup=ReplyKeyboardMarkup(self.text.answ_1))
        self.state = 0
    ...
    def handle_all(self, text, id, context):
        ...
        if self.state == 3: #create program
            ...
            if text != "Набрать массу 💪" and text != "Похудеть 🏃" and text != "maintain weight"  and text !="Назад":
                return context.bot.send_message(chat_id=id, text = self.text.use_button)
            if text == "Набрать массу 💪":
